...
class RespondersControllerTest < ActionController::TestCase
  ...
  def test_create
    assert_difference('Responder.count') do
      post :create, responder: { capacity: responder.capacity, name: responder.name, type: responder.type }
    end
  ...
  def test_update
    put :update, id: responder, responder: { capacity: responder.capacity, name: responder.name, type: responder.type }
    assert_redirected_to responder_path(assigns(:responder))
  end
